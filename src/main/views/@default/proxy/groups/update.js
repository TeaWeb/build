Tea.context(function () {
	 ...
	 */
	this.conds = [];
	if (this.group.conds != null) {
		this.conds = this.group.conds.$map(function (k, v) {
			return {
				"param": v.param,
				"op": v.operator,
				"value": v.value,
	...
	this.ipRanges = [];
	if (this.group.ipRanges != null) {
		this.ipRanges = this.group.ipRanges.$map(function (k, v) {
			if (v.type == 1) { // range
				return {
					"type": "range",
					"from": v.ipFrom,
	...
	this.requestHeaders = [];
	if (this.group.requestHeaders != null) {
		this.requestHeaders = this.group.requestHeaders.$map(function (k, v) {
			return {
				"name": v.name,
				"value": v.value
			};
	...
	if (this.group.responseHeaders != null) {
		this.responseHeaders = this.group.responseHeaders.$map(function (k, v) {
			return {
				"name": v.name,
				"value": v.value
			};
